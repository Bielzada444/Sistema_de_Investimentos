...
void TUDinheiro::testarCenarioValidoDinheiro() {
	try {
		dinheiro->setValor(DINHEIRO_VALIDO); // Tenta definir um Dinheiro valido
		if(dinheiro->getValor() != std::stod(DINHEIRO_VALIDO)) { // Verifica se o valor foi armazenado corretamente
...
void TUDinheiro::testarCenarioInvalidoDinheiro() {
	try {
		dinheiro->setValor(DINHEIRO_INVALIDO); //tenta definir um Dinheiro invalido
		estadoDinheiro = FALHA; // Se conseguir marca como FALHA
	} catch(DominioException &excecao) { // Lanca excecao
		if(dinheiro->getValor() == std::stod(DINHEIRO_INVALIDO)) { // Verifica se o Dinheiro invalido nao foi armazenado
